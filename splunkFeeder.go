...
func (sf SplunkFeeder) Send(pm PublishMetric) {
	sf.MetricLog.Printf("UUID=%v transaction_id=%v publishDate=%v publishOk=%v duration=%v endpoint=%v ",
		pm.UUID, pm.tid, pm.publishDate.UnixNano(), pm.publishOK, pm.publishInterval.upperBound, pm.config.Alias)
}
